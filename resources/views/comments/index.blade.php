...
<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="mt-6 bg-white shadow-sm rounded-lg divide-y">
            ...
            <form class="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8" method="POST" action="{{ route('comments.store', ['post' => $post->id]) }}">
                ...
                <textarea rows="5" name="message" placeholder="{{ __('Digite seu comentário') }}" class=" mx-auto block w-3/4 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm"
                ...
                <div id="codecomment" hidden>
                    ...
                    @if($post->type->value == 'SC')
                        <div class="flex w-5/6 items-start justify-end">
                            <select name="type_id" class=" mt-4 text-sm rounded-md focus:border-blue-500 focus:ring-blue-500 h-10">
                ...
                <div class="bg-white shadow-sm rounded-lg divide-y">
                    ...
                    <div class="p-10 flex-1 space-x-2">
                        ...
                        @if ($mostliked->user->pfp != null)
                            <img class="my-auto h-10 w-10 rounded-md" src=" {{asset("storage/profilepicture/". $mostliked->user->id. "." . $mostliked->user->pfp) }}">
                        @else
                            <img class="my-auto h-10 w-10" src=" {{ asset('img/no-image.svg') }}">
                        @endif
                                ...
                                @if ($mostliked->user->is(auth()->user()))
                                    <x-dropdown>
                                        ...
                                        <x-slot name="content">
                                            ...
                                            <form method="POST" action="{{ route('comments.destroy', $mostliked) }}">
                                                @csrf
                                                @method('delete')
                    ...
                    <div class="flex justify-start mt-5">
                        <form id="mostlikedform" data-comment-id="{{ $mostliked->id }}" class="flex justify-start likeform"
                            onsubmit="togglecomment(event, '{{ route('like.comment', $mostliked) }}', true)">
                ...
                <div class="bg-white shadow-sm rounded-lg divide-y">
                    ...
                    <div class="p-6 flex-1 space-x-2">
                        ...
                        @if ($comment->user->pfp != null)
                            <img class="my-auto h-10 w-10 rounded-md" src=" {{asset("storage/profilepicture/". $comment->user->id. "." . $comment->user->pfp) }}">
                        @else
                            <img class="my-auto h-10 w-10" src=" {{ asset('img/no-image.svg') }}">
                        @endif
